...
exports.submitPharynxCancer = async (req, res) => {
  try {
    ...
    // Save to database
    const record = await db.PharynxCancer.create(formData);

    return res.status(201).json({
      success: true,
      message: "Pharynx cancer data saved successfully",
      recordId: record.id
    });

  } catch (error) {
    console.error("Error saving pharynx cancer data:", error);
    return res.status(500).json({
      success: false,
      message: "Server error while saving pharynx cancer data",
      error: error.message
    });
  }
};